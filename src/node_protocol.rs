...
impl AppMessage {
    pub fn deserialize(root_node: Node) -> Result<AppMessage> {
        let event_type = root_node
            .get_attribute("add")
            .and_then(|add| MessageEventType::from_node(add.as_str()))
            .ok();
        match root_node.desc() {
                        ...
                        match node.desc() {
                            "message" => {
                                if let NodeContent::Binary(ref content) = node.content {
                                    app_events.push(AppEvent::Message(Box::new(
                                        ChatMessage::from_proto_binary(content)?,
                                    )));
                                } else {
                            ...
                            "received" => {
                                app_events.push(AppEvent::MessageAck(MessageAck::from_app_message(
                                    MessageId(node.take_attribute("index")?.into_string()),
                                    MessageAckLevel::from_node(
                                        node.get_attribute("type")?.as_str(),
                                    )?,
                                    node.take_attribute("jid")?.into_jid()?,
                                    node.take_attribute("participant")
                                        .and_then(|participant| participant.into_jid())
                                        .ok(),
                                    node.take_attribute("owner")?
                                        .as_str()
                                        .parse()
                                        .map_err(|_| "NAN")?,
                                )))
                            ...
                            "read" => {
                                ...
                                app_events.push(AppEvent::ChatAction(
                                    jid,
                                    if node
                                        .take_attribute("type")
                                        .ok()
                                        .map_or(true, |typ| typ.as_str() != "false")
                                    {
                                        ChatAction::Read
                                    } else {
                                        ChatAction::Unread
            ...
            "response" => match root_node.get_attribute("type")?.as_str() {
                "contacts" => {
    ...
    pub fn serialize(self, epoch: u32) -> Node {
        ...
        match self {
                            ...
                            .map(|event| {
                                match event {
                                    AppEvent::MessageRead { id, peer } => {
                                        let mut attributes = HashMap::new();
                                        attributes
                                            .insert("index".cow(), NodeContent::String(id.0.cow()));
                                        match peer {
                                            Peer::Individual(jid) => {
                                                attributes
                                                    .insert("jid".cow(), NodeContent::Jid(jid));
                                            }
                                            Peer::Group { group, participant } => {
                                                attributes
                                                    .insert("jid".cow(), NodeContent::Jid(group));
                                                attributes.insert(
                                                    "participant".cow(),
                                                    NodeContent::Jid(participant),
                                                );
                                            }
                                        }
                                        attributes
                                            .insert("owner".cow(), NodeContent::Token("false"));
                                        attributes
                                            .insert("count".cow(), NodeContent::String("1".cow()));
                                    ...
                                    AppEvent::MessagePlayed { id, peer } => {
                                        ...
                                        attributes
                                            .insert("type".cow(), NodeContent::Token("played"));
                                        ...
                                        match peer {
                                            Peer::Individual(jid) => {
                                                attributes
                                                    .insert("from".cow(), NodeContent::Jid(jid));
                                            }
                                            Peer::Group { group, participant } => {
                                                attributes
                                                    .insert("from".cow(), NodeContent::Jid(group));
                                                attributes.insert(
                                                    "participant".cow(),
                                                    NodeContent::Jid(participant),
                                                );
                                    ...
                                    AppEvent::Message(message) => Node::new(
                                        "message",
                                        HashMap::new(),
                                        NodeContent::Binary(message.into_proto_binary()),
                                    ),
                                    AppEvent::GroupCommand {
                                        ...
                                        match command {
                                            GroupCommand::Create(subject) => {
                                                attributes.insert(
                                                    "subject".cow(),
                                                    NodeContent::String(subject.cow()),
                                                );
                                                attributes.insert(
                                                    "type".cow(),
                                                    NodeContent::Token("create"),
                                                );
                                            }
                                            GroupCommand::ParticipantsChange(
                                                jid,
                                                participants_change,
                                            ) => {
                                                attributes.insert(
                                        ...
                                        attributes
                                            .insert("author".cow(), NodeContent::Jid(inducer));
                                        attributes
                                            .insert("id".cow(), NodeContent::String(id.cow()));
                                        Node::new(
                                            "group",
                                            attributes,
                                            NodeContent::List(
                                    ...
                                    AppEvent::PresenceChange(status, jid) => {
                                        ...
                                        attributes.insert(
                                            "type".cow(),
                                            NodeContent::Token(status.into_node()),
                                        );
                                        if let Some(jid) = jid {
                                    ...
                                    AppEvent::ChatAction(jid, action) => {
                                        ...
                                        match action {
                                            ChatAction::Pin(time) => {
                                                attributes.insert(
                                                    "type".cow(),
                                                    NodeContent::String("pin".cow()),
                                                );
                                                attributes.insert(
                                                    "pin".cow(),
                                                    NodeContent::String(time.to_string().cow()),
                                                );
                                            }
                                            //Fixme
                                            ChatAction::Unpin => {
                                            ...
                                            ChatAction::Archive => {
                                                attributes.insert(
                                                    "type".cow(),
                                                    NodeContent::Token("archive"),
                                                );
                                            }
                                            ChatAction::Unarchive => {
                                                attributes.insert(
                                                    "type".cow(),
                                                    NodeContent::Token("unarchive"),
                                                );
                                            }
                                    ...
                                    AppEvent::StatusChange(status) => Node::new(
                                        "status",
                                        HashMap::new(),
                                        NodeContent::String(status.cow()),
                                    ),
            ...
            AppMessage::Query(query) => match query {
...
pub fn parse_message_response(root_node: Node) -> Result<Vec<ChatMessage>> {
    if root_node.desc() == "response"
        && root_node
            .get_attribute("type")
            .ok()
            .map_or(false, |typ| typ.as_str() == "message")
    {
    ...
    fn parse_node(node: &mut Node) -> Result<Contact> {
        Ok(Contact {
            name: node
                ...
                .ok(),
            notify: node
                .take_attribute("notify")
                .map(|notify| notify.into_string())
                .ok(),
    ...
    fn parse_node(node: &mut Node) -> Result<Chat> {
        Ok(Chat {
            ...
            spam: node
                .take_attribute("spam")
                .ok()
                .and_then(|t| t.into_string().parse().ok()),
            mute_until: node
                .take_attribute("mute")
                .ok()
                .and_then(|t| t.into_string().parse().ok()),
            pin_time: node
                ...
                .and_then(|t| t.into_string().parse().ok()),
            read_only: node
                .take_attribute("read_only")
                .ok()
                .and_then(|read_only| read_only.into_string().parse().ok())
                .unwrap_or(false),
        })
    }
}
