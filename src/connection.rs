...
enum SessionState {
    PendingNew {
        ...
        public_key: Vec<u8>,
        client_id: [u8; 8],
        qr_callback: Box<Fn(QrCode) + Send>,
    },
    PendingPersistent {
...
impl<H: WhatsappWebHandler<H> + Send + Sync + 'static> WhatsappWebConnectionInner<H> {
    fn send_json_message(
        &mut self,
        message: JsonValue,
        cb: Box<Fn(JsonValue, &WhatsappWebConnection<H>) + Send>,
    ) {
    ...
    fn send_app_message(
        ...
        cb: Box<Fn(WebsocketResponse, &WhatsappWebConnection<H>) + Send>,
    ) {
        self.epoch += 1;
    ...
    fn ws_send_message(
        ...
        callback: Box<Fn(WebsocketResponse, &WhatsappWebConnection<H>) + Send>,
    ) {
    ...
    fn ws_on_connected(&mut self, out: Sender) {
        ...
        let message: (
            JsonValue,
            Box<Fn(JsonValue, &WhatsappWebConnection<H>) + Send>,
        ) = match self.session_state {
            SessionState::PendingNew { ref client_id, .. } => {
                dbg!("SessionState::PendingNew");
                ...
                (
                    ...
                    Box::new(move |response, connection| {
                        if let Ok(reference) = json_protocol::parse_init_response(dbg!(&response)) {
            ...
            SessionState::PendingPersistent {
                ...
                let mut init_command = json_protocol::build_init_request(
                        ...
                        if let Err(err) = json_protocol::parse_response_status(&response) {
                            ...
                            let message: (
                                ...
                                SessionState::PendingPersistent {
                                    ...
                                    let mut login_command = json_protocol::build_takeover_request(
